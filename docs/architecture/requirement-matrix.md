...
| **R3** Unit не заменяет integration; integration = реальная Payload/MongoDB | — | ECLASS-56 | `tests/integration/classes/class-routes.test.ts` (второе Mongo-подключение читает данные), `tests/integration/auth/*` против replica set | `quality` job, Mongo service | Acceptance proven — main-run [31966275474](https://github.com/ke1evra/eclass/actions/runs/31966275474), PR [#7](https://github.com/ke1evra/eclass/pull/7) |
...
| **R13** Rate limit в общем хранилище: sliding window, fail-closed, без enumeration; ЛИТЕРАЛЬНЫЙ cross-process + неблокирующий login | A2, A3, A5, E4 | ECLASS-59 | `tests/integration/auth/rate-limit.test.ts` (multi-instance, race, 429, privacy, fail-closed, latency-проба, per-code окно против брутфорса кода) + `cross-process-restart.test.ts` (rate-hit/rate-check) | `quality` job | Acceptance proven — main-run [31966275474](https://github.com/ke1evra/eclass/actions/runs/31966275474) |
| **R14** Критический P1-поток A1→T3→A7→S1 без skip | Journey Map, A1–A8, T1–T3, S1–S2 | ECLASS-2/56 | `tests/acceptance/p1-identity-flow.spec.ts` (без skip; student viewport 320px, no-h-scroll) | `e2e` job | Acceptance proven — main-run [31966275474](https://github.com/ke1evra/eclass/actions/runs/31966275474) (эпик ждёт ECLASS-16) |
